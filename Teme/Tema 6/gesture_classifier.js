export default class GestureClassifier {
    ...
    async predict(image, grid) {
        ...
        if (confidences[label] === 1 && label != 'default') {
            ...
            if (!coloredDiv.classList.contains(label)) {
                coloredDiv.classList.remove('no-gesture');
                coloredDiv.classList.add(label);
            }
            setTimeout(() => {
                btn.classList.remove('predicted');
                coloredDiv.classList.remove(label);
                coloredDiv.classList.add('no-gesture');
            }, 500);
        }
        img.dispose();
    }
